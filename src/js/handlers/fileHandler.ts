  ...
  try {
    ...
    if (toolId === 'view-metadata') {
      ...
      try {
        ...
        const createSection = (title) => {
          ...
          ul.className =
            'space-y-3 text-sm bg-(--background) p-4 rounded-lg border border-gray-700';
    ...
    if (toolId === 'edit-metadata') {
      ...
      addBtn.onclick = () => {
        ...
        keyInput.className =
          'custom-meta-key w-1/3 bg-input-bg border border-gray-600 text-white rounded-lg p-2';

        const valueInput = document.createElement('input');
        valueInput.type = 'text';
        valueInput.placeholder = 'Value (e.g., Marketing)';
        valueInput.className =
          'custom-meta-value flex-grow bg-input-bg border border-gray-600 text-white rounded-lg p-2';
