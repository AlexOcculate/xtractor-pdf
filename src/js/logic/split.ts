...
async function renderVisualSelector() {
  ...
  try {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      wrapper.className =
        'page-thumbnail-wrapper p-1 border-2 border-transparent rounded-lg cursor-pointer hover:border-fuchsia-500';
      ...
      const handleSelection = (e: any) => {
        ...
        if (isSelected) {
          wrapper.classList.remove('selected', 'border-fuchsia-500');
          wrapper.classList.add('border-transparent');
        } else {
          wrapper.classList.add('selected', 'border-fuchsia-500');
  ...
  try {
    ...
    switch (splitMode) {
      ...
      case 'bookmarks': {
        ...
        const baseName = state.files[0]?.name.replace(/\.pdf$/i, '') || 'split-by-bookmarks';
      ...
      case 'n-times': {
        ...
        const now = new Date().toISOString();
        const baseName = state.files[0]?.name.replace(/\.pdf$/i, '') || 'split-n-times';
        downloadFile(zipBlob2, undefined, `${now}_${baseName}.zip`);
        hideLoader();
        return;
      }
    }
    ...
    if (
      ...
      const zipBlob = await zip.generateAsync({ type: 'blob' });
      const now = new Date().toISOString();
      const baseName = state.files[0]?.name.replace(/\.pdf$/i, '') || 'split-pages';
      downloadFile(zipBlob, undefined, `${now}_${baseName}.zip`);
    } else {
      const newPdf = await PDFLibDocument.create();
      ...
      const pdfBytes = await newPdf.save();
      const now = new Date().toISOString();
      const filename = state.files[0]?.name ? `${now}_${state.files[0].name}` : `${now}_split-document.pdf`;
      downloadFile(
