...
function renderSavedSignatures() {
  ...
  signState.savedSignatures.forEach((img, index) => {
    ...
    wrapper.className =
      'saved-signature p-1 bg-white rounded-md cursor-pointer border-2 border-transparent hover:border-fuchsia-500 h-16';
  ...
  tabButtons.forEach((button, index) => {
    button.onclick = () => {
      ...
      tabButtons.forEach((btn) => {
        btn.classList.remove('border-fuchsia-500', 'text-white');
        btn.classList.add('border-transparent', 'text-gray-400');
      });
      tabPanels[index].classList.remove('hidden');
      button.classList.add('border-fuchsia-500', 'text-white');
