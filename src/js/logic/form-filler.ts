...
function createFormFieldHtml(field: any): HTMLElement {
  ...
  wrapper.className =
    'form-field-group p-4 bg-input-bg rounded-lg border border-gray-700';
  ...
  if (field instanceof PDFTextField) {
    ...
    input.className =
      'w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5';
    inputElement = input;
  } else if (field instanceof PDFCheckBox) {
    ...
    input.name = name;
    input.className =
      'w-4 h-4 rounded text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500';
