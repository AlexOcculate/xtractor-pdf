...
export async function imageToPdf() {
    ...
    if (type === 'image/jpeg' || type === 'image/jpg') {
      ...
      if (firstFile.name.toLowerCase().endsWith('.heic') ||
        ...
        try {
          ...
          const baseName = state.files[0].name.replace(/\.(jpg|jpeg|png|gif|bmp|webp|tiff|tif|svg|heic|heif)$/i, '');
  ...
  try {
    ...
    const pdfBytes = await pdfDoc.save();
    const now = new Date().toISOString();
    const firstFile = sortedFiles.length > 0 ? sortedFiles[0] : state.files[0];
    const baseName = firstFile.name.replace(/\.(jpg|jpeg|png|gif|bmp|webp|tiff|tif|svg|heic|heif)$/i, '');
